...
def read_json_data(file="./StepThree(data).json"):
...
# Read the data from the StepThree(data).json data file
data=read_json_data()
...
col = ['category', 'app_mini_description', 'appTitle']
df = df[col]
...
predicted_class = []
for item, cat,name in zip(df["app_mini_description"], df["category"], df["appTitle"]):


    prediction = clf.predict(count_vec.transform([item]))
    if prediction[0] != cat:

        print("App Title: {}".format(name))
        print("App Description: {}".format(item))
        print("App Developer Category: {}".format(cat))
        print("App Predicted Category: {}".format(prediction[0]))
